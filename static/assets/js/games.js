...
const load = () => {
  const sortListener = document
    .querySelector("#searchSort")
    .addEventListener("change", () => {
      settingsStorage.set(
        "game_sort",
        document.querySelector("#searchSort").value
      );
      ...
      fetch("/api/games")
        .then((res) => res.json())
        .then((games) => {
          if (settingsStorage.get("game_sort") === "abc")
            games.all.sort((a, b) => a.name.localeCompare(b.name));
          if (settingsStorage.get("game_sort") === "newest")
            games.all.reverse();
  ...
  if (!settingsStorage.get("game_sort"))
    settingsStorage.set("game_sort", "none");

  document.querySelector("#searchSort").value =
    settingsStorage.get("game_sort");
      ...
      searchBar.addEventListener("input", () => {
        if (searchBar.value) {
          ...
          document
            ...
            .forEach((game) => game.classList.remove("hidden"));
          document.querySelector(".searchErr").classList.add("hidden");
        }
      });
