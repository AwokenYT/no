...
const commits = await (
  await fetch(`https://api.github.com/repos/AwokenYT/nostalking`)
).json();
...
const routes = (app) => {
  app.get("/api/analytics/site/:domain", async (req, res, next) => {
    try {
      const request = await fetch(
        (config.options.api.secure ? "https" : "http") +
          "://" +
          config.options.api.domain +
          "/analytics/site/" +
          req.params.domain
      );
  ...
  app.get("/api/changelog", async (req, res) => {
    const changelog = {
      ...
      changelog: JSON.parse(
        fs.readFileSync(
          path.join(__dirname, "../static/assets/JSON/changelog.json")
        )
      ),
    };
        ...
        changelog.commit = {
          ...
          message:
            childProcess
              ...
              .toString()
              .split("\n")[1]
              .replace("changelog ", "") || "unknown",
        };
        changelog.upToDate = commits[0]
          ? commits[0].sha ===
              childProcess.execSync("git rev-parse HEAD").toString().trim() ||
            false
          : false;
  ...
  app.get("/api/favicon", async (req, res) => {
    try {
      const request = await fetch(
        `https://www.google.com/s2/favicons?domain=${req.query.domain}`
      );
      ...
      res.end(
        fs.readFileSync(path.join(__dirname, "../static/assets/img/logo.png"))
      );
    }
  });
  ...
  app.get("/api/games", async (req, res) => {
    try {
      const response = await fetch(
        "https://raw.githack.com/AwokenYT/gamestorage/refs/heads/main/games.json"
      );
    ...
    if (cachedGames[gameId - 1]) {
      ...
      if (URL.canParse(game.image)) {
        try {
          ...
          res.end(
            fs.readFileSync(
              path.join(__dirname, "../static/assets/img/logo.png")
            )
          );
        }
      } else {
  ...
  app.get("/api/apps", (req, res) => {
    const apps = JSON.parse(
      fs.readFileSync(path.join(__dirname, "../static/assets/JSON/apps.json"))
    );
    const appsObject = [];
  ...
  app.get("/api/cheats", (req, res) => {
    const cheats = JSON.parse(
      fs.readFileSync(path.join(__dirname, "../static/assets/JSON/cheats.json"))
    );
    const cheatsObject = [];
  ...
  app.get("/api/registerVisit", async (req, res) => {});
};
